...
const Navbar: React.FC<NavbarProps> = ({ onNavigate, currentPath }) => {
  ...
  const navLinks = [
    { name: 'Home', path: 'home' },
    { name: 'About', path: 'home', hash: '#hero' },
    { name: 'Portfolio', path: 'home', hash: '#portfolio' },
    { name: 'Services', path: 'home', hash: '#services' },
    { name: 'Pricing', path: 'home', hash: '#pricing' },
    { name: 'Blog', path: 'blog' },
    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled || currentPath === 'blog' ? 'py-3 bg-white/90 dark:bg-background-dark/90 backdrop-blur-md shadow-lg border-b border-subtle-light dark:border-subtle-dark' : 'py-6 bg-transparent'}`}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <button 
            ...
            className="hidden md:block bg-primary hover:bg-primary-hover text-white px-5 py-2 rounded-full text-sm font-bold transition-all transform hover:scale-105"
          >
            START PROJECT
          </button>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white dark:bg-surface-dark absolute w-full left-0 p-6 space-y-4 shadow-xl border-b border-subtle-light dark:border-subtle-dark">
          ...
          <button 
            onClick={handleStartProject}
            className="w-full bg-primary hover:bg-primary-hover text-white py-3 rounded-lg text-sm font-bold uppercase tracking-widest"
          >
            Start Project
          </button>
        </div>
